...
The `AsFd` implementation `assert!`s `fd >= 0` before calling `unsafe { BorrowedFd::borrow_raw(fd) }`. `BorrowedFd::borrow_raw` forbids `-1` (and is meaningless for `i32::MIN` uninit), so the assert converts misuse into a deterministic panic instead of silent UB. The remaining half of the safety contract — that the fd stays open for the borrow's lifetime — is the caller's responsibility; in practice the internal callers either (a) hold their own `BorrowedFd<'h>` already (see [entry-ext.md](entry-ext.md)) and don't go through `DirFd::as_fd` at all, or (b) hold the `DirFd` inside a live `DirHandle`, which keeps the underlying `nix::Dir` alive.
